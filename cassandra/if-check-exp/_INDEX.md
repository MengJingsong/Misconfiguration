...
|--------|-------|--------|---------|----------------|--------|------|
| `memtable` | `memtable_heap_space` | `bytebuffer` | (b) | [`SubPool.tryAllocate():156`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/utils/memory/MemtablePool.java#L156) | verified | [link](memtable/tryAllocate-limit-memtable_heap_space.md) |
| `memtable` | `memtable_offheap_space` | `region` | (b) | [`SubPool.tryAllocate():156`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/utils/memory/MemtablePool.java#L156) | verified | [link](memtable/tryAllocate-limit-memtable_offheap_space.md) |
| `net` | `internode_application_receive_queue_capacity` | `message` | (b) | [`AbstractMessageHandler.acquireCapacity():419`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/net/AbstractMessageHandler.java#L419) | pending | [link](net/acquireCapacity-queueCapacity-internode_application_receive_queue_capacity.md) |
| `net` | `native_transport_receive_queue_capacity` | `message` | (b) | [`AbstractMessageHandler.acquireCapacity():419`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/net/AbstractMessageHandler.java#L419) | pending | [link](net/acquireCapacity-queueCapacity-native_transport_receive_queue_capacity.md) |
| `hints` | `HintsBufferPool_MAX_ALLOCATED_BUFFERS` | `hintsbuffer` | (a) | [`HintsBufferPool.switchCurrentBuffer():113`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/hints/HintsBufferPool.java#L113) | pending | [link](hints/switchCurrentBuffer-MAX_ALLOCATED_BUFFERS-MAX_HINT_BUFFERS.md) |
| `commitlog` | `cdc_total_space` | `allocation` | (b) | [`CDCSizeTracker.processNewSegment():335`](https://github.com/apache/cassandra/blob/cassandra-5.0.9/src/java/org/apache/cassandra/db/commitlog/CommitLogSegmentManagerCDC.java#L335) | pending | [link](commitlog/processNewSegment-allowance-cdc_total_space.md) |
...
- **`tryAllocate-limit-memtable_heap_space`:** hard allocation cap in `SubPool.tryAllocate()`, gating `ByteBuffer.allocate()` for memtable writes. Verified via new `HeapPoolTest` unit test (2026-09-16).
- **`tryAllocate-limit-memtable_offheap_space`:** sibling case, same `SubPool.tryAllocate()` if-check on the `offHeap` `SubPool`, reached via `NativeAllocator` — gates off-heap `Region`/native memory allocation instead of `ByteBuffer`. Note: accounting call is decoupled from the physical allocation call (see case notes). Verified via existing `NativeAllocatorTest.testBookKeeping()` (2026-09-16).
